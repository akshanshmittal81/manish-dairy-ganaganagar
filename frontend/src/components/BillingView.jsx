        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3, width: 72, flexShrink: 0, maxHeight: "calc(100vh - 160px)", overflowY: "auto" }}>
          {SIDEBAR_CATS.map((c) => (
            <button key={c} onClick={() => setCategory(c)}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2, padding: "6px 4px", borderRadius: 10, border: "2px solid", borderColor: category === c ? (CAT_COLORS[c] || "#f59e0b") : "#e5e0d8", background: category === c ? (CAT_COLORS[c] || "#f59e0b") : "#fff", color: category === c ? "#fff" : "#4a3f35", fontSize: 10, fontWeight: category === c ? 800 : 500, cursor: "pointer", transition: "all 0.15s", boxShadow: category === c ? `0 4px 12px ${(CAT_COLORS[c] || "#f59e0b")}44` : "none" }}>
        ...
        {cart.length > 0 && (
          <div style={{ padding: "12px 14px", borderTop: "2px dashed #e5e0d8" }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
              {["CASH", "UPI"].map((mode) => (
                <button key={mode} onClick={() => setPaymentMode(mode)}
                  style={{ flex: 1, padding: "9px", borderRadius: 10, border: "2px solid", borderColor: paymentMode === mode ? "#f59e0b" : "#e5e0d8", background: paymentMode === mode ? "#1a1310" : "#fff", color: paymentMode === mode ? "#f59e0b" : "#8a7e6e", fontWeight: 800, fontSize: 13, cursor: "pointer" }}>
                  {mode === "CASH" ? "💵 CASH" : "📲 UPI"}
                </button>
              ))}
            </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={() => { checkoutBill(paymentMode); setPaymentMode("CASH"); }} style={{ flex: 1, height: 42, borderRadius: 10, background: "#1a1310", color: "#f59e0b", border: "none", fontWeight: 800, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
